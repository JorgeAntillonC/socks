		...
		case sk_ndtp_ops: {
			...
			for (u8 i = 0; i < cnode->opsc; i++) {
				...
				switch (ndop->type) {
							...
							switch (ndop->op) {
								...
								case skvm_op_call: {
									...
									if (procaddr->prim != sk_tp_proc && procaddr->prim != sk_tp_eproc) {
										SK_ErrorAtNode(cu, ndop, "an uncallable address reach the call");
									...
									if (vstacksize >= calledtypes->argsc) {
										...
										SK_ErrorAtNode(cu, ndop, "not enough operands to call { %$ } expected %4u but recived %4u", procstr->str, (u8)procstr->len, calledtypes->argsc, vstacksize);
										vstack->occ = 0;
					...
					case sk_ndtp_dmlay: {
						// the checker ignores this node type, but it is need it for the bytecode gen stage
					} break;
