...
static inline SK_STR* SK_StrFromSymoff(SK_COMPUNIT* cu, u4 symoff);
...
static inline SK_STR* SK_StrFromSymoff(SK_COMPUNIT* cu, u4 symoff) {
	...
	return (SK_STR*)DB_Index(cu->strs->pool, sym->stroff);
}
...
inline static u8 SK_Node2StrSimp(SK_COMPUNIT* cu, SK_NODE* node, u1* buff) {
	switch (node->type) {
		case sk_ndtp_str: {
			u1 strbuff[512];
			if (node->symf) {
				SK_STR* strte = SK_StrFromSymoff(cu, node->symoff);
		...
		case sk_ndtp_smlay:
		case sk_ndtp_dmlay: {
			SK_STR* strte = SK_StrFromSymoff(cu, node->mlaysym);
			return CstrFmt(buff, "(%s:%$)", sk_ndtp2str[node->type], strte->str, (u8)strte->len);
		}
		case sk_ndtp_cast:
		case sk_ndtp_type: {
	...
	switch (node->type) {
		case sk_ndtp_str: {
			...
			if (node->strf) {
				...
				CstrFromRawBytes(node->strlit->data, node->strlit->occ, strbuff, 512);
				return CstrFmt(buff, "(%s:%s)", sk_ndtp2str[node->type], strbuff);
			}
		} break;
		case sk_ndtp_symdef:
...
inline static void SK_NodePrintInfo(SK_COMPUNIT* cu, SK_NODE* node) {
	...
	SK_STR* scstr = SK_StrFromSymoff(cu, cu->currentcompunit);
	Print("%s at %$:%u:%u\n", buff, scstr->str, (u8)scstr->len, (u8)node->row + 1, (u8)node->col + 1);
}
...
static void SK_ErrorAtNode(SK_COMPUNIT* cu, SK_NODE* node, u1* errmsg, ...) {
	...
	if (node) {
		if (errmsg) {
			...
			errlen = CstrFmt(errbuff, "\x1B[31mERROR\x1B[37m::%s at %$:%2u:%2u\n\n", errbuff + 512, scstr->str, (u8)scstr->len, cnode->row + 1, cnode->col + 1);
		}
		else {
			SK_Node2StrSimp(cu, node, errbuff + 512);
			errlen = CstrFmt(errbuff, "\x1B[31mERROR\x1B[37m::rogue %s at %$:%2u:%2u\n\n", errbuff + 512, scstr->str, (u8)scstr->len, cnode->row + 1, cnode->col + 1);
		}
	}
	else {
		VAR_ARG(errmsg, stack);
		CstrFmtS(errbuff + 512, errmsg, stack);
		errlen = CstrFmt(errbuff, "\x1B[31mERROR\x1B[37m::%s at %$\n\n", errbuff + 512, scstr->str, (u8)scstr->len);
	}
